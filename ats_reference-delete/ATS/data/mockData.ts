...
export interface Activity {
  id: string;
  candidateId: string;
  type: "stage_change" | "note" | "email" | "interview" | "rating";
  message: string;
  timestamp: string;
  user: string;
}
...
export const defaultCriteria: ScorecardCriterion[] = [
...
export function useAtsData() {
  ...
  const [scoresList, setScoresList] = useState<CandidateScore[]>(candidateScores);
  const [templatesList, setTemplatesList] = useState<ScorecardTemplate[]>(scorecardTemplates);
  ...
  const updateCandidateStage = (id: string, stage: Stage) => {
  ...
  const addCandidate = (candidate: Omit<Candidate, "id" | "avatar" | "appliedDate">) => {
  ...
  const submitScore = (score: Omit<CandidateScore, "id">) => {
  ...
  const updateScoreTemplate = (jobId: string, criteria: ScorecardCriterion[]) => {
  ...
  const getWeightedScore = (candidateId: string) => {
  ...
  const getCandidateRanking = (jobId: string) => {
