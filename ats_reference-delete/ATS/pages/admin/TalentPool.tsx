...
interface TalentProfile {
  ...
  source: string;
  addedDate: string;
  status: "active" | "contacted" | "interested" | "not_interested";
  matchScore: number;
  notes: string;
}
...
const TalentPool = () => {
  ...
  const allTags = [...new Set(talents.flatMap((t) => t.tags))];
  ...
  const updateStatus = (id: string, status: TalentProfile["status"]) => {
          ...
          <Card key={talent.id} className="hover:shadow-md transition-shadow">
            <CardContent className="p-4 space-y-3">
              ...
              <div className="flex items-center justify-between pt-1 border-t border-muted">
                ...
                <Select value={talent.status} onValueChange={(v) => updateStatus(talent.id, v as TalentProfile["status"])}>
