...
import { Send, AtSign, MessageSquare, Clock, ThumbsUp, Reply } from "lucide-react";
...
interface NoteEntry {
  ...
  replies: { text: string; author: string; authorAvatar: string; timestamp: Date }[];
}

const CollaborativeNotes = ({ candidateId, existingNotes }: { candidateId: string; existingNotes: string[] }) => {
  ...
  const toggleReaction = (noteId: string, emoji: string) => {
