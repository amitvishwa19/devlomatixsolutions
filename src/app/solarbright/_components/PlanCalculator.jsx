...
import calculatorConfig from "../_data/calculatorConfig";
        ...
        <div className="max-w-5xl mx-auto grid lg:grid-cols-5 gap-8">
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <RadioGroup value={frequency} onValueChange={setFrequency} className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {calculatorConfig.cleaningFrequency.map((opt) => (
                  <Label
                    ...
                    className={`flex items-center gap-3 p-3 rounded-xl border cursor-pointer transition-all duration-200 ${
                      frequency === opt.value
                        ? "border-primary/50 bg-primary/5 shadow-glow shadow-primary/10"
          ...
          >
            ...
            <div className="rounded-2xl border border-border bg-card p-6 space-y-4">
              ...
              <div className="space-y-3 text-sm">
                <div className="flex justify-between text-foreground">
                  <span className="text-muted-foreground font-light">Total Panels</span>
                  <span className="font-bold text-primary text-lg">{totalPanels}</span>
                </div>
                <div className="flex justify-between text-foreground">
                  <span className="text-muted-foreground font-light">Cleanings / Year</span>
                  <span className="font-bold">{frequencyMultiplier}x</span>
                </div>
                <div className="h-px bg-border" />
                <div className="flex justify-between text-foreground">
                  <span className="text-muted-foreground font-light">Est. Per Visit</span>
                  <span className="font-bold">₹{(totalPanels * recommendedPlan.pricePerPanel).toLocaleString()}</span>
                </div>
                <div className="flex justify-between text-foreground">
                  <span className="text-muted-foreground font-light">Est. Annual Cost</span>
            ...
            <div className="rounded-2xl border border-primary/40 bg-card p-6 space-y-4 shadow-glow relative overflow-hidden">
              <div className="absolute top-0 right-0 flex items-center gap-1 bg-primary text-primary-foreground px-3 py-1 rounded-bl-xl text-[10px] font-bold uppercase tracking-wider">
              ...
              <div className="flex items-baseline gap-1">
                ...
                <span className="text-muted-foreground text-sm font-light">/panel</span>
              </div>
              <p className="text-sm text-muted-foreground font-light">{recommendedPlan.description}</p>
                ...
                .map((plan) => (
                  <div key={plan.id} className="rounded-xl border border-border bg-card p-4 flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground font-light">₹{plan.pricePerPanel}/panel</p>
                    </div>
