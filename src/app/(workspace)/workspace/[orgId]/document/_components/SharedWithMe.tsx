...
const permissionColors = {
  view: "bg-secondary text-muted-foreground border-border",
  edit: "bg-primary/10 text-primary border-primary/20",
  admin: "bg-chart-3/10 text-chart-3 border-chart-3/20",
};

export function SharedWithMe({ documents, onView, onDownload }: SharedWithMeProps) {
  if (documents.length === 0) {
    return (
      <div className="bg-card rounded-xl border border-border p-8 text-center">
        <div className="h-14 w-14 rounded-full bg-secondary/50 flex items-center justify-center mx-auto mb-4">
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    <Badge 
                      variant="outline" 
                      className={cn("text-xs font-normal", permissionColors[doc.permission])}
                    >
                ...
                <div className="flex items-center gap-2">
                  <Avatar className="h-6 w-6">
                    <AvatarFallback className="bg-primary/10 text-primary text-[10px]">
                      {doc.sharedBy.name.split(" ").map(n => n[0]).join("").toUpperCase()}
                    </AvatarFallback>
                  </Avatar>
                  <span className="text-xs text-muted-foreground">
                    Shared by <span className="text-foreground">{doc.sharedBy.name}</span>
                  </span>
                </div>
