...
import { User, TestTube, Calendar, Stethoscope, FileText, Printer } from 'lucide-react';
...
export function OrderDetailsDialog({ order, open, onOpenChange, onUpdateOrder }) {
  ...
  const handlePrint = useReactToPrint({
    contentRef: printRef,
    documentTitle: `Lab-Report-${order?.orderId}`,
  });
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="bg-card border-border max-w-2xl">
        ...
        <div className="hidden">
          <PrintableResult ref={printRef} order={order} />
        </div>
      </DialogContent>
    </Dialog>
  );
