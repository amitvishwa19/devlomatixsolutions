...
export default function AppointmentEditorCopy
    ({ isOpen, appointment, onClose, data }) {
    ...
    useEffect(() => {
        console.log(appointment?.patientDetails?.id)
        setAppointmentData({
            patientId: appointment?.patientDetails?.id || null,
            doctorId: null,
            serverId: orgId,
            date: moment().format(),
            slot: hospitalDefaultSettings?.timing[0],
        ...
        <Sheet open={isOpen} onOpenChange={handleOpenChange}>
            <SheetContent className='min-w-[620px] bg-transparent border-l-0 p-2'>
                ...
                <div className='bg-card h-full rounded-md'>
                    ...
                    <SheetFooter className='flex flex-row item  justify-end'>
                        <SheetClose asChild>
                            <Button variant="outline" size={'sm'} onClick={() => { handleOpenChange() }}>Close</Button>
                        </SheetClose>
                        <Button variant="save" size={'sm'} onClick={handleSaveData}>
