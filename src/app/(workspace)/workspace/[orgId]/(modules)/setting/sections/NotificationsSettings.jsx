...
export function NotificationsSettings() {
  ...
  useEffect(() => {
    if (appSettings?.notifications) {
      form.reset({
        // Email Notifications
        emailAppointmentConfirmations: appSettings.notifications.emailAppointmentConfirmations || false,
        emailAppointmentReminders: appSettings.notifications.emailAppointmentReminders || false,
        emailInvoiceBilling: appSettings.notifications.emailInvoiceBilling || false,
        emailLabResults: appSettings.notifications.emailLabResults || false,

        // SMS Notifications
        smsAppointmentReminders: appSettings.notifications.smsAppointmentReminders || false,
        smsPrescriptionReady: appSettings.notifications.smsPrescriptionReady || false,
        smsPaymentConfirmations: appSettings.notifications.smsPaymentConfirmations || false,

        // In-App Notifications
        inAppNewPatient: appSettings.notifications.inAppNewPatient || false,
        inAppEmergencyAlerts: appSettings.notifications.inAppEmergencyAlerts || false,
        inAppLowInventory: appSettings.notifications.inAppLowInventory || false,
        inAppScheduleChanges: appSettings.notifications.inAppScheduleChanges || false,

        // Schedule
        quietHoursStart: appSettings.notifications.quietHoursStart || "",
        quietHoursEnd: appSettings.notifications.quietHoursEnd || "",
      });
    } else {
      form.reset({
        emailAppointmentConfirmations: false,
        emailAppointmentReminders: false,
        emailInvoiceBilling: false,
        emailLabResults: false,
        smsAppointmentReminders: false,
        smsPrescriptionReady: false,
        smsPaymentConfirmations: false,
        inAppNewPatient: false,
        inAppEmergencyAlerts: false,
        inAppLowInventory: false,
        inAppScheduleChanges: false,
        quietHoursStart: "",
        quietHoursEnd: "",
      });
    }
  ...
  const { execute } = useAction(upsertGeneralSetting, {
    onSuccess: (data) => {
      setLoading(false)
      toast.success('Notifications settings saved successfully', { id: 'notification' })
    },
    onError: (error) => {
      console.log(error)
      setLoading(false)
      toast.error('Oops somethig went wrong ! try again later', { id: 'notification' })
      setLoading(false);
    }
  })
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      const toastId = toast.loading("Saving Notifications settings...", { id: 'notification' });
      await execute({ userId: session.user.userId, type: 'notifications', payload: data })
