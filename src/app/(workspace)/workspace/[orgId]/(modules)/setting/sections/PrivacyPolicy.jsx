...
export function PrivacyPolicy() {
    ...
    const form = useForm({
        ...
        defaultValues: {
            privacyPolicy: '<p>privacy policy</p>', // initial editor content
        },
    });

    useEffect(() => {
        if (!appSettings?.privacy) return;

        form.reset({
            privacyPolicy: appSettings.privacy, // ✅ HTML from backend
        });
    ...
    const { execute } = useAction(upsertGeneralSetting, {
        ...
        onError: (error) => {
            console.log(error)
            setLoading(false)
            toast.error('Oops somethig went wrong ! try again later', { id: 'privacy' })
            setLoading(false);
        }
    })
    ...
    const onSubmit = async (data) => {
        ...
        try {
            setLoading(true)
            toast.loading("Saving privacy policy, please wait...", { id: 'privacy' });
            await execute({ userId: session.user.userId, type: 'privacy', payload: data.privacyPolicy })
        } catch (error) {
