...
export function SecuritySettings() {
    ...
    useEffect(() => {
        if (appSettings?.security) {
            form.reset({
                // Password Policy
                minPasswordLength: appSettings.security.minPasswordLength || "",
                passwordExpiry: appSettings.security.passwordExpiry || "",
                requireUppercase: appSettings.security.requireUppercase || false,
                requireNumbers: appSettings.security.requireNumbers || false,
                requireSpecialChars: appSettings.security.requireSpecialChars || false,

                // Authentication
                twoFactorAuth: appSettings.security.twoFactorAuth || false,
                sessionTimeout: appSettings.security.sessionTimeout || "",
                failedLoginLockout: appSettings.security.failedLoginLockout || "",
            });
        } else {
            form.reset({
                minPasswordLength: "",
    ...
    const { execute } = useAction(upsertGeneralSetting, {
        onSuccess: (data) => {
            setLoading(false)
            toast.success('Security settings saved successfully', { id: 'security' })
        },
        onError: (error) => {
            console.log(error)
            setLoading(false)
            toast.error('Oops somethig went wrong ! try again later', { id: 'security' })
            setLoading(false);
        }
    })
    ...
    const onSubmit = async (data) => {
        ...
        try {
            setLoading(true)
            const toastId = toast.loading("Saving security settings...", { id: 'security' });
            await execute({ userId: session.user.userId, type: 'security', payload: data })
        } catch (error) {
