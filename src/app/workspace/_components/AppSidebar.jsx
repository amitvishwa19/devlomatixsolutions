    ...
    return (
        <div>
            <Sidebar collapsible="icon" className="  border-r border-border/50">
                        ...
                        {state === "expanded" ? (
                            ...
                            >
                                <Image
                                    ...
                                    width={32}
                                    height={32}
                                    className="rounded-md object-contain shadow-glow-sm"
                                />
                            </motion.div>
                        )}
                    </AnimatePresence>
                    ...
                    <ScrollArea className="h-[75vh]">
                        {navigation.map((item, index) => {
                            ...
                            const isGroupActive =
                                pathname === baseUrl ||
                                child.some((c) => {
                                    if (c.url === `/workspace/${workspaceId}`)
                                        return pathname === c.url;
                                    return pathname === c.url || pathname.startsWith(c.url + "/");
                                });
                            return (
                                <SidebarGroup
                                    key={index}
                                    className="p-0 group-data-[collapsible=icon]:ml-2"
                                >
                                                    ...
                                                    <div className="flex flex-row items-center justify-between w-full pr-2">
                                                        <div className="flex flex-row items-center gap-2">
                                                            <DynamicIcon
                                                                name={parent.icon}
                                                                size={18}
                                                                className={`shrink-0 transition-colors ${isGroupActive ? "text-primary" : "text-muted-foreground"}`}
                                                            />
                                                            <span
                                                                className={`opacity-90 text-xs transition-all ${isGroupActive ? "font-semibold text-glow" : ""}`}
                                                            >
                                                                {parent.title}
                                                            </span>
                                                        </div>
                                                        <ChevronRight
                                                            size={14}
                                                            className={`transition-transform duration-300 text-muted-foreground ${isOpen ? "rotate-90" : ""}`}
                                                        />
                                                    </div>
                                                ) : (
                                                    <Link
                                                        href={parent.url}
                                                        className="flex flex-row items-center justify-between w-full pr-2"
                                                    >
                                                        <div className="flex flex-row items-center gap-2">
                                                            <DynamicIcon
                                                                name={parent.icon}
                                                                ...
                                                                <SidebarMenuItem key={i}>
                                                                    ...
                                                                    >
                                                                        ...
                                                                        >
                                                                            ...
                                                                            <span
                                                                                className={`text-xs transition-all duration-300 group-data-[collapsible=icon]:hidden ${isActive ? "text-foreground" : "text-muted-foreground group-hover/sub:text-primary/70"}`}
                                                                            >
                                                                                {c.title}
                                                                            </span>
                ...
                <div className="mt-auto px-2 pb-2">
                    <div className="flex flex-col gap-1 p-2 rounded-xl bg-primary/5 border border-primary/10 backdrop-blur-sm group-data-[collapsible=icon]:p-1 group-data-[collapsible=icon]:items-center">
                        <Link
                            href={`/workspace/${workspaceId}/system/docs`}
                        ...
                        <div
                            ...
                            onClick={() => {
                                console.log('Setting modal clicked');
                                setSettingModal({ ...settingModal, open: true });
                            }}
                        >
                            <DynamicIcon
                                name="settings"
            ...
            <SettingModal
                ...
                onClose={() => {
                    setSettingModal({ ...settingModal, open: false });
                }}
            />
        </div>
    );
