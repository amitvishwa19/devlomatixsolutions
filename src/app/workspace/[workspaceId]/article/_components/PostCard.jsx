            ...
            <AlertDialog open={isDeleteDialogOpen} onOpenChange={setIsDeleteDialogOpen}>
                <AlertDialogContent className="rounded-xl border border-border shadow-2xl overflow-hidden p-0 animate-fade-in">
                    ...
                    <AlertDialogFooter className="p-8 pt-4 flex flex-row gap-4 bg-muted/20">
                        <AlertDialogCancel className="rounded-md text-[10px] font-bold tracking-widest flex-1 mt-0 border-border/60">Cancel</AlertDialogCancel>
                        <AlertDialogAction 
                            onClick={(e) => {
                                e.preventDefault();
                                handleDelete();
                            }}
                            className="rounded-md text-[10px] font-bold tracking-widest flex-1 bg-rose-500 hover:bg-rose-600 text-white shadow-lg shadow-rose-500/20 pointer-events-auto"
                            disabled={isDeleting}
                        >
                    ...
                    <div className="flex flex-wrap gap-1.5 mb-3">
                        ...
                        {(post.tags || []).map((tag, i) => (
                            <Badge 
                                key={i} 
                                variant="outline" 
                                className="text-[9px] font-bold text-muted-foreground/60 tracking-widest px-2 py-0.5 rounded-md border-border/100 bg-muted/5"
                            >
                                #{tag}
                            </Badge>
                        ))}
