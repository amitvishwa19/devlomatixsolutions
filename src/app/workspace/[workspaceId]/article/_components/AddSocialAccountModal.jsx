...
export const AddSocialAccountModal = () => {
    ...
    const onSubmit = async (e) => {
        ...
        try {
            const payload = {
                platform,
                accessToken,
                profileName: profileName || `${platform} Account`,
    ...
    return (
        <Dialog open={isModalOpen} onOpenChange={handleClose}>
            <DialogContent className="sm:max-w-md bg-background border border-border/100 rounded-lg shadow-2xl p-0 overflow-hidden">
                <form onSubmit={onSubmit} className="flex flex-col">
                    ...
                    <div className="p-8 pt-0 space-y-6">
                        <div className="p-4 bg-primary/5 border border-primary/10 rounded-md flex items-start gap-3">
                            ...
                            <p className="text-[10px] font-bold text-muted-foreground tracking-tight leading-relaxed text-left">
                        ...
                        <div className="space-y-3 text-left">
                            <label className="text-[10px] font-bold tracking-widest text-muted-foreground opacity-70 ml-1">Platform</label>
                        ...
                        <div className="space-y-4 text-left">
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold tracking-widest text-muted-foreground opacity-70 ml-1">Access Token</label>
                                <div className="relative">
                                    ...
                                    <Input
                                        ...
                                        className="pl-12 bg-muted/30 border-none rounded-md focus-visible:ring-1 focus-visible:ring-primary shadow-inner h-12"
                                    />
                                </div>
                            </div>

                            <div className="space-y-2">
                                <label className="text-[10px] font-bold tracking-widest text-muted-foreground opacity-70 ml-1">Profile Name (Display Only)</label>
                                <Input
                                    disabled={isLoading}
                                    placeholder="e.g. Acme Corp Official"
                                    value={profileName}
                                    onChange={(e) => setProfileName(e.target.value)}
                                    className="bg-muted/30 border-none rounded-md focus-visible:ring-1 focus-visible:ring-primary shadow-inner h-12"
                                />
                            </div>
                        </div>
                    </div>
                    ...
                    <DialogFooter className="p-8 bg-muted/10 border-t border-border/10">
                        <Button
                            ...
                            variant="ghost"
                            onClick={handleClose}
                            className="rounded-md font-bold text-muted-foreground"
                        >
                            Cancel
                        </Button>
                        <Button
                            type="submit"
                            disabled={isLoading}
                            className="bg-primary hover:bg-primary/90 min-w-[140px] rounded-md font-bold shadow-lg shadow-primary/20"
                        >
                            {isLoading ? (
