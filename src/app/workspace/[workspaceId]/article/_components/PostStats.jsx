            ...
            {stats.map((stat, i) => (
                <div key={i} className="bg-card/100 p-6 rounded-lg border border-border/100 backdrop-blur-md hover:border-primary/20 transition-colors">
                    <div className="flex items-center justify-between mb-2">
                        ...
                        <Badge variant="outline" className="border-none bg-muted/30 text-[10px] tracking-widest text-muted-foreground font-bold">This Month</Badge>
                    </div>
                    <h4 className="text-2xl font-bold text-foreground">{stat.value}</h4>
                    <p className="text-muted-foreground text-[10px] font-bold tracking-widest">{stat.label}</p>
                </div>
            ))}
        </div>
    );
