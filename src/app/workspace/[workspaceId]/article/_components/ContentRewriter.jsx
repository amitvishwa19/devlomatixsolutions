...
export const ContentRewriter = () => {
    ...
    return (
        <div className="pt-6 px-4 pb-12 w-full max-w-5xl mx-auto">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                ...
                <Card className="border-border shadow-sm bg-card">
                    <CardHeader className="pb-3 border-b border-border/30 mb-3 bg-secondary/10">
                        <CardTitle className="text-base text-foreground">Original Content</CardTitle>
                    </CardHeader>
                    <CardContent className="space-y-4">
                        <Textarea
                            ...
                            className="min-h-[180px] text-sm bg-secondary/50 focus-visible:ring-primary"
                        />
                        <div className="grid grid-cols-2 gap-3">
                            <Select value={tone} onValueChange={setTone}>
                                <SelectTrigger className="bg-secondary/50"><SelectValue placeholder="Tone" /></SelectTrigger>
                                <SelectContent>
                                    {tones.map((t) => (
                            ...
                            <Select value={platform} onValueChange={setPlatform}>
                                <SelectTrigger className="bg-secondary/50"><SelectValue placeholder="Platform (optional)" /></SelectTrigger>
                                <SelectContent>
                        ...
                        <Select value={language} onValueChange={setLanguage}>
                            <SelectTrigger className="bg-secondary/50"><SelectValue placeholder="Language" /></SelectTrigger>
                            <SelectContent>
                                {languages.map((l) => (
                        ...
                        />
                        <Button onClick={rewrite} disabled={loading || !content.trim()} className="w-full h-11 text-base font-semibold">
                ...
                {/* Output */}
                <Card className={`border-border shadow-sm bg-card transition-opacity ${result ? "opacity-100" : "opacity-50"}`}>
                    <CardHeader className="pb-3 border-b border-border/30 mb-3 bg-secondary/10 flex flex-row items-center justify-between">
                        <CardTitle className="text-base text-foreground">Rewritten Content</CardTitle>
                        {result && (
                            <Button variant="outline" size="sm" onClick={copyResult} className="h-8">
                    ...
                    <CardContent className="space-y-4">
                        {result ? (
                            <>
                                <div className="rounded-md bg-secondary/30 p-4 text-sm whitespace-pre-wrap min-h-[180px] text-foreground border border-border/30">
                                ...
                                {result.improvement_score > 0 && (
                                    <div className="space-y-2 pt-2">
                                        <div className="flex justify-between text-xs font-semibold text-muted-foreground">
                                            ...
                                            <span className="text-primary">{result.improvement_score}%</span>
                                        </div>
                                        <Progress value={result.improvement_score} className="h-2 bg-secondary" />
                                    </div>
                                )}
                                {result.changes_summary && (
                                    <p className="text-xs text-muted-foreground italic border-l-2 border-primary/50 pl-2 mt-4">{result.changes_summary}</p>
                                )}
                            </>
                        ) : (
                            <div className="flex flex-col items-center justify-center min-h-[220px] text-muted-foreground/60 text-sm gap-3">
