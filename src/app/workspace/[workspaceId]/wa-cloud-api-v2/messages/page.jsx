  ...
  return (
    <Card className="rounded-md border-border/60 bg-card shadow-sm">
      <CardHeader className="flex flex-row items-center justify-between border-b border-border/60 py-4">
        <div>
          <CardTitle className="text-lg font-bold">Message Stream</CardTitle>
          <p className="text-xs text-muted-foreground font-medium">Real-time log of all inbound and outbound events.</p>
        </div>
        <div className="flex items-center gap-3">
          <Badge variant="outline" className="text-[10px] font-bold h-5">{filtered.length} MESSAGES</Badge>
      ...
      <CardContent className="space-y-4 p-4">
        <div className="grid gap-3 rounded-md border border-border/60 bg-muted/20 p-3 md:grid-cols-4">
          <div className="md:col-span-2">
            ...
            <div className="relative">
              ...
              <Input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Phone, name, content..." className="pl-9 h-9" />
            </div>
          </div>
          <div>
          ...
          <ScrollArea className="h-[calc(100vh-320px)]">
            <table className="w-full text-xs">
              ...
              <tbody className="divide-y divide-border/60">
                {loading && !rows.length && (
                  <tr><td colSpan={7} className="py-20 text-center text-muted-foreground animate-pulse">Fetching stream...</td></tr>
                )}
                {filtered.map((r) => {
