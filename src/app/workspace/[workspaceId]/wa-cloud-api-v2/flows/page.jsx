...
import { ClipboardList, Plus, Trash2, ExternalLink, Copy, Wand2, Check, FileCode, Loader2, Send } from "lucide-react";
    ...
    try {
      ...
      if (res.flow_id) {
        ...
        if (res.published) {
          ...
          toast.warning(`Flow created (ID ${res.flow_id}) but auto-publish failed. Publish manually in Meta.`);
        }
      } else {
      ...
      <DialogContent className="w-[80vw] max-w-[80vw] h-[80vh] max-h-[80vh] p-0 flex flex-col overflow-hidden sm:rounded-lg">
        <DialogHeader className="px-6 pt-6">
          ...
          <p className="text-xs text-muted-foreground">
            Choose the fields you want to collect. We'll generate the Flow JSON for Meta.
          </p>
        </DialogHeader>
            ...
            <div className="grid flex-1 min-h-0 gap-6 lg:grid-cols-[1fr_360px]">
              ...
              <div className="flex flex-col rounded-lg border border-border/60 bg-card/30 p-4 min-h-0">
                ...
                <ScrollArea className="flex-1 min-h-0 rounded-md bg-muted/30 p-3">
                  <div className="mx-auto w-full max-w-xs rounded-2xl bg-card p-3 shadow-sm ring-1 ring-border/60">
                      ...
                      {selected
                        ...
                        .map((f) => (
                          <div key={f.key} className="space-y-1">
                            ...
                            <div className="h-7 rounded-md border border-dashed border-border/60 bg-muted/40" />
                          </div>
                        ))}
          ...
          <TabsContent value="publish" className="mt-4 flex-1 min-h-0">
            <div className="grid h-full min-h-0 gap-6 lg:grid-cols-[1fr_320px]">
              <div className="flex min-h-0 flex-col">
                ...
                <ScrollArea className="flex-1 min-h-0 rounded-md border border-border/60 bg-muted/30 p-3">
              ...
              <div className="space-y-4">
                <div className="rounded-md border border-border/60 bg-card/30 p-3 text-[11px] leading-relaxed text-muted-foreground">
                  ...
                  <ol className="list-inside list-decimal space-y-1">
                    <li>Open Meta WhatsApp Manager → Flows.</li>
                    <li>Create flow, name it, choose "Without endpoint".</li>
                    <li>Paste the JSON on the left.</li>
                    <li>Save → Publish. Copy Flow ID.</li>
                    <li>Paste below and Save flow.</li>
                  </ol>
...
export default function FlowsPage() {
  ...
  const [viewJsonFor, setViewJsonFor] = useState(null);
    ...
    <div className="space-y-6">
        ...
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-xs text-muted-foreground">
              Register published Meta Flows here to use them in message templates.
            </p>
          </div>
          ...
          {flows.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-lg border border-dashed border-border/60 bg-muted/20 py-12 text-center">
              ...
              <p className="mt-1 max-w-md text-xs text-muted-foreground">
                Design a signup flow in seconds, or register an existing Meta Flow ID.
              </p>
            ...
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {flows.map((f) => (
                <div
                  key={f.flow_id}
                  className="group flex flex-col rounded-lg border border-border/60 bg-card/40 p-4 transition-all hover:border-primary/50"
                >
                  ...
                  {f.description && (
                    <p className="mt-2 line-clamp-2 text-[11px] text-muted-foreground">{f.description}</p>
                  )}
      ...
      <Dialog open={!!viewJsonFor} onOpenChange={(o) => !o && setViewJsonFor(null)}>
        <DialogContent className="max-w-2xl">
          ...
          <ScrollArea className="max-h-[60vh] rounded-md border border-border/60 bg-muted/30 p-3">
            <pre className="whitespace-pre-wrap break-all font-mono text-[11px] leading-relaxed">
