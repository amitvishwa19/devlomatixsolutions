        ...
        <div className="space-y-8 max-w-7xl mx-auto py-6 px-4 animate-fade-in text-foreground">
            ...
            {loading ? (
                <div className="flex flex-col items-center justify-center py-40 space-y-4">
                    ...
                    <p className="text-[10px] font-bold text-muted-foreground tracking-widest animate-pulse">Mapping role hierarchy...</p>
                </div>
            ) : filteredRoles.length === 0 ? (
                <div className="py-24 text-center border-2 border-dashed border-border/40 rounded-3xl bg-muted/5">
                    <Shield className="w-12 h-12 text-muted-foreground/30 mx-auto mb-4" />
                    <h3 className="text-lg font-bold tracking-tight">No identities found</h3>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredRoles.map((role) => (
                        <Card key={role.id} className="group relative border-border overflow-hidden rounded-lg bg-card/30 backdrop-blur-md hover:shadow-2xl hover:shadow-blue-500/5 hover:border-blue-500/40 transition-all duration-500 shadow-sm">
                            {/* Accent Bar */}
                            ...
                            <CardHeader className="pt-8 px-8">
                                <div className="flex items-center justify-between mb-4">
                                    <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-background/50 border border-border/20">
                                        ...
                                        <span className="text-[10px] font-bold tracking-widest">{role._count?.users || 0} Members</span>
                                    </div>
                                    <Badge variant="outline" className="rounded-md border-none bg-blue-500/10 text-blue-500 font-bold text-[9px] tracking-tight">
                                        ID: {role.id.slice(-4).toUpperCase()}
                                    </Badge>
                                </div>
                                <CardTitle className="text-2xl font-bold tracking-tight">{role.title}</CardTitle>
                            ...
                            <CardContent className="px-8 py-4">
                                <div className="space-y-4">
                                    <div className="flex items-center justify-between text-[10px] font-bold tracking-widest text-muted-foreground/50 border-b border-border/10 pb-2">
                                        <span>Permissions</span>
                                        <span>{role.permissions.length} Tokens</span>
                                    </div>
                                    <div className="flex flex-wrap gap-2 min-h-[40px]">
                                        {role.permissions.slice(0, 4).map(p => (
                                            <Badge key={p.id} variant="secondary" className="bg-muted/40 text-[9px] font-bold rounded-lg border-none hover:bg-muted/60">
                                                {p.title}
                                            </Badge>
                                        ))}
                            ...
                            <CardFooter className="px-8 pb-8 pt-4">
                                <Button
                                    ...
                                    className="w-full h-11 rounded-xl font-bold text-[10px] tracking-widest bg-background/40 hover:bg-blue-50 border-border/40 hover:border-blue-500/40 hover:text-blue-600 transition-all shadow-sm"
                                >
            ...
            <Dialog open={isCreateModalOpen} onOpenChange={setIsCreateModalOpen}>
                <DialogContent className="sm:max-w-md rounded-[2.5rem] border-none shadow-2xl p-8 bg-card">
                    <DialogHeader>
                        <DialogTitle className="text-2xl font-bold tracking-tight">{selectedRole ? "Configure" : "Initialize"} Identity</DialogTitle>
                    ...
                    <div className="py-6 space-y-5">
                        <div className="grid grid-cols-2 gap-4">
                            <div className="space-y-2">
                                <label className="text-[10px] font-bold tracking-widest text-muted-foreground ml-1">Role Title</label>
                                <Input
                        ...
                        <div className="space-y-3 pt-2">
                            <label className="text-[10px] font-bold tracking-widest text-muted-foreground ml-1">Capability Matrix</label>
                                ...
                                {Object.keys(permissions).length === 0 ? (
                                    ...
                                    Object.entries(permissions).map(([category, perms]) => (
                                        <div key={category} className="space-y-2">
                                            <p className="text-[9px] font-bold tracking-widest text-blue-500/70 border-b border-blue-500/10 pb-1">{category}</p>
                                                ...
                                                {perms.map(perm => (
                                                    ...
                                                    >
                                                        <div className="min-w-0">
                                                            <p className="text-[11px] font-bold tracking-tight truncate">{perm.title}</p>
