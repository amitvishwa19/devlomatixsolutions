            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-card/10 p-8 rounded-lg border border-border/40 backdrop-blur-sm shadow-sm">
                ...
                <div className="flex gap-3">
                    <Button
                        ...
                        variant="outline"
                        className="rounded-md font-bold h-11 px-6 text-[10px] border-border/40 tracking-widest"
                    >
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="border-b border-border/20 bg-muted/10">
                                    <th className="px-6 py-5 text-[10px] font-bold tracking-widest text-muted-foreground whitespace-nowrap">Member Details</th>
                                    <th className="px-6 py-5 text-[10px] font-bold tracking-widest text-muted-foreground whitespace-nowrap">Access Rights</th>
                                    <th className="px-6 py-5 text-[10px] font-bold tracking-widest text-muted-foreground whitespace-nowrap">Directory Status</th>
                                    <th className="px-6 py-5 text-[10px] font-bold tracking-widest text-muted-foreground whitespace-nowrap">Member Since</th>
                                    ...
                                    <tr key={user.id} className="group hover:bg-primary/[0.02] transition-colors">
                                        <td className="px-6 py-5 whitespace-nowrap">
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div className="min-w-0">
                                                    ...
                                                    <p className="text-[10px] font-bold text-muted-foreground truncate opacity-70 tracking-widest">
                                                        @{user.username || 'n/a'}
                                                    </p>
                                                </div>
                                            </div>
                                        ...
                                        <td className="px-6 py-5">
                                            <div className="flex flex-wrap gap-1.5 max-w-[200px]">
                                                {user.roles.length === 0 ? (
                                                    <span className="text-[10px] font-bold text-muted-foreground opacity-30 tracking-widest">No assigned roles</span>
                                                ) : (
                                                    user.roles.map(role => (
                                                        <Badge
                                                            key={role.id}
                                            ...
                                            <div className="flex items-center gap-2">
                                                {user.isActive ? (
                                                    ...
                                                    <div className="flex items-center gap-1.5 text-orange-500 bg-orange-500/10 px-2.5 py-1 rounded-md border border-orange-500/20">
                                                        ...
                                                        <span className="text-[10px] font-bold tracking-widest">Idle</span>
                                                    </div>
                                                )}
                                            </div>
                                        </td>
                                        <td className="px-6 py-5 whitespace-nowrap">
                                            <p className="text-[10px] font-bold text-muted-foreground opacity-80 tracking-widest">
                                        ...
                                        <td className="px-6 py-5 whitespace-nowrap text-right">
                                            <DropdownMenu>
                                                ...
                                                <DropdownMenuContent align="end" className="w-52 rounded-lg shadow-2xl border-border/40 p-2">
                                                    <DropdownMenuLabel className="text-[10px] font-bold tracking-widest text-muted-foreground px-3 py-2">Account Control</DropdownMenuLabel>
                                                    <DropdownMenuItem onClick={() => openEditModal(user)} className="cursor-pointer font-bold text-[10px] tracking-widest px-3 py-2.5 rounded-md">
                                                        <Shield className="w-4 h-4 mr-2 text-primary" /> Manage Roles
                                                    </DropdownMenuItem>
                                                    <DropdownMenuItem onClick={() => toggleStatus(user)} className="cursor-pointer font-bold text-[10px] tracking-widest px-3 py-2.5 rounded-md">
                                                        {user.isActive ? (
                                                    ...
                                                    <DropdownMenuItem onClick={() => confirmDelete(user)} className="cursor-pointer font-bold text-[10px] tracking-widest px-3 py-2.5 rounded-md text-rose-500 hover:bg-rose-500/10 transition-colors">
                ...
                <DialogContent className="sm:max-w-md bg-background border border-border/60 shadow-2xl rounded-lg overflow-hidden p-0">
                    <div className="p-8 pb-4">
                        <DialogHeader>
                            ...
                            <DialogDescription className="text-[10px] font-bold tracking-widest text-muted-foreground opacity-70">
                                Set access levels and directory status for <span className="text-primary tracking-tight">@{selectedUser?.username}</span>.
                            </DialogDescription>
                        </DialogHeader>
                        ...
                        <div className="py-8 space-y-6">
                            <div className="space-y-4 border-b border-border/10 pb-8">
                                <div className="space-y-2 text-left">
                                    <label className="text-[10px] font-bold tracking-widest text-muted-foreground ml-1">Display Name</label>
                                    <Input
                                        value={selectedUser?.displayName || ''}
                                        onChange={(e) => setSelectedUser(prev => ({ ...prev, displayName: e.target.value }))}
                                        className="h-12 bg-muted/30 border-none rounded-md px-4 text-xs font-bold tracking-tight shadow-inner focus-visible:ring-1 focus-visible:ring-primary"
                                    />
                                </div>
                                <div className="flex items-center justify-between p-4 rounded-md bg-muted/20 border border-border/10">
                                    <div className="space-y-1">
                                        <p className="text-[10px] font-bold tracking-widest">Directory Access</p>
                                    ...
                                    <Button
                                        ...
                                        className={`h-9 rounded-md font-bold tracking-widest text-[9px] px-4 ${selectedUser?.isActive ? 'border-emerald-500/20 text-emerald-500 hover:bg-emerald-500/5' : ''}`}
                                    >
                            ...
                            <div className="space-y-4 text-left">
                                <label className="text-[10px] font-bold tracking-widest text-muted-foreground ml-1">Assign Roles</label>
                                <div className="grid grid-cols-1 gap-2 max-h-[220px] overflow-y-auto pr-2">
                                    {roles.length === 0 ? (
                                        <p className="text-center py-4 text-[10px] text-muted-foreground tracking-widest font-bold">No roles defined yet.</p>
                                    ) : (
                                        roles.map((role) => (
                                            <div
                                                key={role.id}
                                            ...
                                            >
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div>
                                                        <p className="text-[10px] font-bold tracking-widest">{role.title}</p>
                    ...
                    <DialogFooter className="p-8 bg-muted/5 border-t border-border/10 flex gap-2">
                        <Button variant="ghost" onClick={() => setIsEditModalOpen(false)} className="rounded-md font-bold px-6 text-[10px] flex-1 tracking-widest">
                            Cancel
                        </Button>
                        <Button
                            onClick={handleUpdateRoles}
                            disabled={isUpdating}
                            className="rounded-md font-bold px-8 shadow-lg shadow-primary/20 bg-primary hover:bg-primary/90 transition-all flex-1 text-[10px] tracking-widest"
                        >
