...
function WorkflowCanvasInner({ workflowId, workflowName: controlledName, initialName, onNameChange, loadedNodes, loadedEdges, initialCron: initCron, initialScheduleEnabled: initSchedule, initialViewport }) {
    ...
    useEffect(() => {
        ...
        // Small timeout to ensure React Flow is ready to receive viewport updates
    ...
    // Automatically close chat if the trigger node is deleted
    useEffect(() => {
        if (!hasChatTrigger && showChat) {
            setShowChat(false);
        }
        ...
        <div className="flex-1 relative h-full flex flex-col" ref={reactFlowWrapper}>
            ...
            <div className="flex-1 relative overflow-hidden pt-12">
                <ReactFlow
                    ...
                    nodeTypes={nodeTypes}
                    edgeTypes={edgeTypes}
                    defaultEdgeOptions={{ type: "workflowEdge", animated: true, style: { stroke: "hsl(var(--n8n-connection))", strokeWidth: 6, strokeDasharray: "2 2" } }}
                >
                ...
                {/* Overlays / Sidebars (Inside relative container for clipping) */}
                <AnimatePresence>
                    {showNodePanel && (
