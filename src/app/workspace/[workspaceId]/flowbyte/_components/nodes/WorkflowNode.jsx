...
const StatusIndicator = ({ status }) => {
  if (status === "success") {
    return (
      <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-emerald-500 border-2 border-background flex items-center justify-center shadow-sm">
  ...
  if (status === "error") {
    return (
      <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-destructive border-2 border-background flex items-center justify-center shadow-sm">
  ...
  if (status === "running") {
    return (
      <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-primary border-2 border-background flex items-center justify-center shadow-sm">
  ...
  if (status === "waiting") {
    return (
      <div className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-amber-500 border-2 border-background flex items-center justify-center shadow-sm animate-pulse">
...
const WorkflowNode = ({ data, selected }) => {
  ...
  const colorClass = colorMap[data.type] || "bg-primary";
  const isAgent = data.type === "ai-agent";
  ...
  const isSubComponent = AI_SUB_TYPES.has(data.type);

  const status = data.status || (data.error ? "error" : data.executed ? "success" : "idle");
  ...
  const isTrigger = ["trigger", "webhook", "schedule", "email-trigger", "chat-trigger", "trigger-placeholder"].includes(data.type);
  const isPlaceholder = data.type === "trigger-placeholder";
  const isChatTrigger = data.type === "chat-trigger";

  if (isPlaceholder) {
    return (
      <div className="flex flex-col items-center gap-2 cursor-pointer group">
        <div className="w-[72px] h-[72px] rounded-xl border-2 border-dashed border-muted-foreground/40 group-hover:border-primary bg-card flex items-center justify-center transition-colors">
    ...
    return (
      <div className="flex flex-col items-center gap-1.5 cursor-pointer group">
        <div className="relative">
          <div
            className={`w-[72px] h-[72px] rounded-2xl border bg-n8n-node-bg flex items-center justify-center transition-all
              ${selected ? "ring-[1px] ring-primary shadow-lg border-primary" : `${borderClass} hover:shadow-lg`}
            `}
          >
          ...
          <Handle type="source" position={Position.Right} className="!w-3 !h-3 !bg-muted-foreground/40 !border-2 !border-background hover:!bg-primary transition-colors" />
        </div>
  ...
  if (isAgent) {
    return (
      <div className="flex flex-col items-center cursor-pointer group">
        <div
          className={`relative rounded-xl border bg-card transition-all min-w-[200px]
            ${selected ? "ring-[1px] ring-primary border-primary" : `${borderClass} hover:shadow-elevated`}
          `}
        >
          <Handle type="target" position={Position.Left} className="!w-3 !h-3 !bg-muted-foreground/40 !border-2 !border-background hover:!bg-primary transition-colors" />
        ...
        <div className="flex items-start gap-6 mt-0">
          {AGENT_SLOTS.map((slot) => {
            const attached = data.attachedSlots?.[slot.id];
            const isBuiltinActive = (slot.id === "llm" && hasBuiltinLlm) || (slot.id === "memory" && hasBuiltinMemory);
            const builtinLabel = slot.id === "llm" && hasBuiltinLlm ? data.config?.builtinLlm || "gpt-4" : slot.id === "memory" && hasBuiltinMemory ? data.config?.builtinMemory : undefined;
            ...
            return (
              <div key={slot.id} className="flex flex-col items-center relative">
                ...
                <div className="relative w-5 h-5 flex items-center justify-center">
                  ...
                  <Handle type="target" position={Position.Bottom} id={`slot-${slot.id}`} className="!w-5 !h-5 !bg-transparent !border-0 !rounded-none !transform-none" style={{ position: "absolute", top: 0, left: 0 }} />
                </div>
                <span className="text-[11px] text-muted-foreground mt-1.5 whitespace-nowrap">
                  {slot.label}
                  {!isBuiltinActive && slot.required && !attached && <span className="text-destructive">*</span>}
                </span>
                {isBuiltinActive && builtinLabel && <span className="text-[9px] text-emerald-500 font-medium truncate max-w-[80px]">{builtinLabel}</span>}
                <div className="w-px h-3 bg-muted-foreground/30" />
                <button
                  onClick={(e) => { e.stopPropagation(); data.onSlotAdd?.(slot.id); }}
  ...
  return (
    <div
      className={`workflow-node rounded-xl border bg-card transition-all cursor-pointer min-w-[160px]
        ${selected ? "ring-[1px] ring-primary shadow-elevated border-primary" : `${borderClass} hover:shadow-elevated`}
      `}
    >
      {!isTrigger && <Handle type="target" position={Position.Left} className="!w-3 !h-3 !bg-muted-foreground/40 !border-2 !border-background hover:!bg-primary transition-colors" />}
      {isSubComponent && <Handle type="source" position={Position.Top} id="to-agent" className="!w-3 !h-3 !bg-violet-400 !border-2 !border-background hover:!bg-violet-500 transition-colors" />}

      <div className="flex items-center gap-3 px-3 py-2.5">
        <div className={`${colorClass} rounded-md p-1.5 flex items-center justify-center`}>
