...
export default function CredentialPage() {
 ...
 className="bg-primary hover:bg-primary/90 text-primary-foreground rounded-xl font-extrabold text-[10px] h-10 px-6 shadow-soft transition-all active:scale-95"
 >
 ...
 <div>
 <p className="text-[10px] font-black text-emerald-500 mb-1">Encrypted Storage</p>
 ...
 <div className="flex items-start justify-between">
 <div>
 <p className="text-[10px] font-black text-primary mb-1">Total Connections</p>
 ...
 <div className="flex items-start justify-between">
 <div>
 <p className="text-[10px] font-black text-blue-500 mb-1">Security Health</p>
 ...
 <p className="text-[10px] font-black tracking-[0.2em] text-muted-foreground animate-pulse">Decrypting Vault...</p>
 </div>
 </div>
 ...
 <h3 className="text-xl font-black ">No credentials found</h3>
 ...
 variant="outline"
 onClick={() => onOpen("addCredential", { workspaceId, onApply: fetchAccounts })}
 className="mt-6 border-dashed border-primary/20 hover:border-primary/40 text-primary font-bold text-[10px] rounded-xl px-8"
 >
 Click to add
 </Button>
 </div>
 ...
 <Button variant="ghost" size="icon" className="w-8 h-8 rounded-xl text-muted-foreground/50 hover:text-foreground hover:bg-muted opacity-0 group-hover:opacity-100 transition-opacity">
 ...
 <p className="text-[9px] font-black text-muted-foreground opacity-40">Stored Data</p>
 <div className="flex flex-wrap gap-1.5">
 {Object.keys(account.details || {}).filter(k => k !== 'profileName').map(key => (
 <Badge key={key} variant="secondary" className="bg-muted/30 border-none text-[9px] font-bold px-2 py-0.5 rounded-md text-muted-foreground/60">
 {key}
 </Badge>
 ))}
 </div>
 ...
 <AlertDialogCancel className="rounded-xl text-[10px] font-bold flex-1 mt-0 border-border/60">Cancel</AlertDialogCancel>
 <AlertDialogAction
 onClick={(e) => {
 e.preventDefault();
 handleDelete();
 }}
 className="rounded-xl text-[10px] font-bold flex-1 bg-rose-500 hover:bg-rose-600 text-white shadow-lg shadow-rose-500/20 pointer-events-auto"
 disabled={isDeleting}
 >
