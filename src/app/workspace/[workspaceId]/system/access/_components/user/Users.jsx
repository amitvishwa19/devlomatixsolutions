    ...
    const columns = [
            ...
            cell: ({ row }) => {
                return (
                    <div className='flex flex-row items-center gap-2 flex-wrap'>
                        <div>{row.original.departments?.length === 0 && <Badge status='blank'>No Department Assigned</Badge>}</div>
                        {row.original.departments?.map((department, index) => (
                            <Badge key={index} status='success'>{department.name}</Badge>
                        ))}
                    </div>
                )
            }
                ...
                return (
                    <div className='flex flex-row items-center gap-2 flex-wrap'>
                        ...
                        {row.original.roles?.map((role, index) => (
                            <Badge key={index} status='info'>{role.title}</Badge>
                        ))}
                    </div>
                )
            }
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className=''>
                        <Badge status={row.original.status ? 'success' : 'na'}>
