...
export function NavigationPermissionForm() {
    ...
    return (
        <ScrollArea className="flex-1 h-[75vh] p-6">
                                ...
                                {group.children.length > 0 ? (
                                    group.children.map((childTitle) => (
                                        <div key={childTitle} className="flex items-center gap-3 group/item p-1">
                                            <Checkbox id={`nav-${group.category}-${childTitle}`} className="rounded-sm" />
                                            <Label
                                                htmlFor={`nav-${group.category}-${childTitle}`}
                                    ...
                                    <div className="flex items-center gap-3 p-1">
                                        <Checkbox id={`nav-${group.category}`} className="rounded-sm" />
                                        <Label
                                            htmlFor={`nav-${group.category}`}
