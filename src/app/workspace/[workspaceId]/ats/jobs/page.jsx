    ...
    <div className="flex flex-col gap-4 p-4 animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-4 p-2 rounded-md bg-card/30 backdrop-blur-xl border border-border/40 shadow-xl shadow-black/5">
        <div className="relative flex-1 w-full text-zinc-900 border-none border-0">
          ...
          <Input
            ...
            className="pl-11 bg-transparent border-none h-12 text-sm font-medium focus-visible:ring-0 shadow-none uppercase text-[10px] tracking-widest opacity-60"
          />
        </div>
      ...
      <div className="bg-card/30 backdrop-blur-xl border border-border/40 rounded-md overflow-hidden shadow-2xl shadow-black/10">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-border/40 bg-muted/10">
                <th className="p-6 text-[10px] tracking-[0.2em] text-muted-foreground opacity-40 uppercase">Position</th>
                <th className="p-6 text-[10px] tracking-[0.2em] text-muted-foreground opacity-40 uppercase">Department</th>
                <th className="p-6 text-[10px] tracking-[0.2em] text-muted-foreground opacity-40 uppercase">Location</th>
                <th className="p-6 text-[10px] tracking-[0.2em] text-muted-foreground opacity-40 text-center uppercase">Applicants</th>
                <th className="p-6 text-[10px] tracking-[0.2em] text-muted-foreground opacity-40 uppercase">Status</th>
                  ...
                  >
                    <td className="p-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-[10px] font-black uppercase text-muted-foreground/60 tracking-wider font-mono">{job.type}</p>
                        </div>
                      </div>
                    </td>
                    <td className="p-6">
                      <Badge variant="outline" className="text-[10px] font-black uppercase tracking-wider border-border/40 bg-muted/20 px-3 py-1 rounded-md">
                        {job.category?.name || job.department || 'General'}
                      </Badge>
                    </td>
                    <td className="p-6">
                      <div className="flex items-center gap-2 text-[10px] font-black uppercase tracking-wider text-muted-foreground opacity-60">
                    ...
                    <td className="p-6 text-center">
                      <div className="inline-flex flex-col items-center justify-center min-w-[60px] p-2 rounded-md bg-muted/20 border border-border/10">
                        ...
                        <span className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/40">Total</span>
                      </div>
                    </td>
                    <td className="p-6">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${getDotColor(job.status)}`} />
                        <span className={`text-[10px] font-black uppercase tracking-wider ${getTextColor(job.status)}`}>
                          {job.status}
                        </span>
                      </div>
                    </td>
                    <td className="p-6">
                      <div className="flex items-center justify-end gap-2">
                        <Button
                          ...
                          size="sm"
                          className="h-8 px-4 rounded-md bg-primary text-primary-foreground text-[9px] font-black uppercase tracking-widest shadow-lg shadow-primary/20 opacity-0 group-hover:opacity-100 transition-all scale-95 group-hover:scale-100"
                        >
                          View Pipeline
                        </Button>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48 rounded-md border-border/40 bg-card/90 backdrop-blur-xl">
                            ...
                            <DropdownMenuItem className="text-[10px] font-black uppercase tracking-widest p-3 gap-2 text-rose-500">
                              Close Position
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
        ...
        <div className="p-6 border-t border-border/40 bg-muted/5 flex items-center justify-between">
          <p className="text-[10px] font-black uppercase tracking-widest text-muted-foreground/40">Showing {displayJobs.length} total positions</p>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" disabled className="h-8 rounded-md text-[10px] font-black uppercase tracking-widest border-border/40 opacity-50">Previous</Button>
            <Button variant="outline" size="sm" disabled className="h-8 rounded-md text-[10px] font-black uppercase tracking-widest border-border/40 opacity-50">Next</Button>
          </div>
        </div>
      </div>
    </div>
