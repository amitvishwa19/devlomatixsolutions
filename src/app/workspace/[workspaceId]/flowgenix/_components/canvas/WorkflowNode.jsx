...
const WorkflowNode = ({ data, selected }) => {
    ...
    return (
      <div className="flex flex-col items-center gap-1.5 cursor-pointer group">
        <div className="relative">
          <div className={`w-[72px] h-[72px] rounded-2xl border bg-card flex items-center justify-center transition-all ${selected ? "ring-[1px] ring-primary shadow-lg border-primary" : `${borderClass} hover:shadow-lg`}`}>
            <MessageSquare className="h-7 w-7 text-foreground" />
          </div>
          <StatusIndicator status={status} />
          <Handle type="source" position={Position.Right} className="!w-3 !h-3 !bg-muted-foreground/40 !border-2 !border-card hover:!bg-primary transition-colors" />
        </div>
        <span className="text-xs text-center text-foreground font-medium max-w-[120px] leading-tight mt-1">{nodeData.label}</span>
      </div>
    );
  }
  ...
  if (isAgent) {
    return (
      <div className="flex flex-col items-center cursor-pointer group">
        <div className={`relative rounded-xl border bg-card transition-all min-w-[200px] ${selected ? "ring-[1px] ring-primary border-primary" : `${borderClass} hover:shadow-lg`}`}>
          <Handle type="target" position={Position.Left} className="!w-3 !h-3 !bg-muted-foreground/40 !border-2 !border-card hover:!bg-primary transition-colors" />
          <div className="flex items-center gap-3 px-4 py-2.5">
            <div className="bg-muted rounded-lg p-1.5 flex items-center justify-center">
              <Bot className="h-5 w-5 text-foreground" />
            </div>
            <span className="text-sm font-semibold text-card-foreground">{nodeData.label}</span>
          </div>
          <StatusIndicator status={status} />
          <Handle type="source" position={Position.Right} className="!w-3 !h-3 !bg-muted-foreground/40 !border-2 !border-card hover:!bg-primary transition-colors" />
        </div>
        <div className="flex items-start gap-6 mt-0">
          {AGENT_SLOTS.map((slot) => {
            ...
            const builtinLabel = slot.id === "llm" && hasBuiltinLlm ? (agentConfig?.builtinLlm || "gpt-4") : (slot.id === "memory" && hasBuiltinMemory ? agentConfig?.builtinMemory : undefined);
            return (
              <div key={slot.id} className="flex flex-col items-center relative">
                ...
                <span className="text-[11px] text-muted-foreground mt-1.5 whitespace-nowrap">{slot.label}</span>
                ...
                <button onClick={(e) => { e.stopPropagation(); nodeData.onSlotAdd?.(slot.id); }} className="w-6 h-6 rounded-md border border-muted-foreground/30 bg-card flex items-center justify-center hover:border-primary hover:text-primary transition-colors text-muted-foreground">
  ...
  return (
    <div className={`workflow-node rounded-xl border bg-card transition-all cursor-pointer min-w-[160px] ${selected ? "ring-[1px] ring-primary shadow-lg border-primary" : `${borderClass} hover:shadow-lg`}`}>
      {!isTrigger && <Handle type="target" position={Position.Left} className="!w-3 !h-3 !bg-muted-foreground/40 !border-2 !border-card hover:!bg-primary transition-colors" />}
      {isSubComponent && <Handle type="source" position={Position.Top} id="to-agent" className="!w-3 !h-3 !bg-violet-400 !border-2 !border-card hover:!bg-violet-500 transition-colors" />}
      <div className="flex items-center gap-3 px-3 py-2.5">
        <div className={`${colorClass} rounded-md p-1.5 flex items-center justify-center`}>
          <Icon className="h-4 w-4 text-primary-foreground" />
        </div>
        <div className="flex flex-col min-w-0">
          <span className="text-sm font-medium text-card-foreground truncate">{nodeData.label}</span>
          {nodeData.subtitle && <span className="text-xs text-muted-foreground truncate">{nodeData.subtitle}</span>}
        </div>
      </div>
      <StatusIndicator status={status} />
      <Handle type="source" position={Position.Right} className="!w-3 !h-3 !bg-muted-foreground/40 !border-2 !border-card hover:!bg-primary transition-colors" />
