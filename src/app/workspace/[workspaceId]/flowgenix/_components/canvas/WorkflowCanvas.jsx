...
function WorkflowCanvasInner({ 
    ...
    useEffect(() => {
        const viewportToRestore = (initialViewport && typeof initialViewport === 'object') ? initialViewport : DEFAULT_VIEWPORT;
        const timer = setTimeout(() => { setViewport(viewportToRestore, { duration: 800 }); }, 100);
    ...
    useEffect(() => { if (!hasChatTrigger && showChat) setShowChat(false); }, [hasChatTrigger, showChat]);

    const chatTriggerNode = nodes.find((n) => (n.data).type === "chat-trigger");
    const agentNode = nodes.find((n) => (n.data).type === "ai-agent");
    const isChatConnectedToAgent = !!(chatTriggerNode && agentNode && edges.some((e) => e.source === chatTriggerNode.id && e.target === agentNode.id));
    ...
    const styledEdges = edges.map((edge) => {
        ...
        if (isWaitingForChat) {
            return {
                ...
                style: { stroke: "var(--chart-4)", strokeWidth: 3, strokeDasharray: "6 6", filter: "drop-shadow(0 0 4px var(--chart-4))" },
            };
        }
        return {
            ...edge, type: 'workflowEdge', data: { status: edgeStatus }, selected: edge.id === selectedEdge,
            style: { ...edge.style, ...(edge.id === selectedEdge ? { stroke: "var(--destructive)", strokeWidth: 3, filter: "drop-shadow(0 0 6px var(--destructive))" } : {}) }
        };
    });
    ...
    const onConnect = useCallback((params) => setEdges((eds) => addEdge({ ...params, type: "workflowEdge", animated: true }, eds)), [setEdges]);
    const onEdgeClick = useCallback((_, edge) => { setSelectedNode(null); setContextMenu(null); setSelectedEdge(edge.id); }, []);
    const deleteEdge = useCallback((edgeId) => { setEdges((eds) => eds.filter((e) => e.id !== edgeId)); setSelectedEdge(null); }, [setEdges]);

    const onNodeClick = useCallback((_, node) => {
        if (node.data.type === "trigger-placeholder") { setShowNodePanel(true); return; }
        setSelectedNode(node); setSelectedEdge(null); setContextMenu(null);
    }, []);
    ...
    const saveWorkflow = useCallback(async () => {
        setIsSaving(true);
        try {
            const res = await saveWorkflowAction({ id: workflowId, name: workflowName, nodes, edges, workspaceId, viewport: getViewport() });
            if (res.error) toast.error(res.error); else toast.success("Workflow saved successfully");
        } catch (error) { toast.error("Failed to save workflow"); } finally { setIsSaving(false); }
    ...
    const handleChatSend = useCallback(async (text) => {
        ...
        if (connectedAgentNode) {
            ...
            let fullResponse = "";
            await streamChat({
                messages: newHistory, model: agentConfig?.builtinLlm || "gpt-4", temperature: (agentConfig?.temperature) ?? 0.7,
                systemPrompt: (agentConfig?.systemPrompt) || "You are a helpful AI assistant.", enableTools,
                onDelta: (chunk) => { fullResponse += chunk; setChatMessages((prev) => prev.map((m) => m.id === botMsgId ? { ...m, text: fullResponse } : m)); },
                onDone: () => {
    ...
    const handleNewSession = useCallback(() => { setChatSessionId(crypto.randomUUID()); setChatMessages([]); setConversationHistory([]); }, []);
    const handleClearExecution = useCallback(() => { setChatLogs([]); }, []);
    const handleLoadTemplate = useCallback((name, templateNodes, templateEdges) => { setNodes(templateNodes); setEdges(templateEdges); }, [setNodes, setEdges]);

    return (
        <div className="flex-1 relative h-full flex flex-col bg-n8n-canvas-bg" ref={reactFlowWrapper}>
            {/* Compact Toolbar */}
            <div className="absolute top-0 left-0 right-0 z-20 flex items-center justify-between px-4 h-12 bg-card/80 glass border-b border-border/60">
            ...
            <div className="flex-1 relative overflow-hidden pt-12">
                ...
                <AnimatePresence>
                    {showNodePanel && <NodePanel onClose={() => setShowNodePanel(false)} onAddNode={addNode} slotFilter={nodePanelSlotFilter} />}
                    {selectedNode && <NodeDetailPanel node={selectedNode} executionResult={executionResults.get(selectedNode.id)} onClose={() => setSelectedNode(null)} onUpdateConfig={onUpdateConfig} />}
                </AnimatePresence>

                {showTemplates && <TemplateGallery onClose={() => setShowTemplates(false)} onLoadTemplate={handleLoadTemplate} />}
            </div>

            <AnimatePresence>
                {showChat && (
                    <motion.div initial={{ y: "100%" }} animate={{ y: 0 }} exit={{ y: "100%" }} transition={{ type: "spring", damping: 25 }} className="h-[400px] z-30">
                        <ChatPanel sessionId={chatSessionId} messages={chatMessages} onSendMessage={handleChatSend} onClose={() => setShowChat(false)} onNewSession={handleNewSession} logs={chatLogs} onClearExecution={handleClearExecution} isStreaming={isStreaming} enableTools={enableTools} onToggleTools={setEnableTools} />
                    </motion.div>
                )}
            </AnimatePresence>

            {contextMenu && (
                <div className="fixed z-50 min-w-[160px] rounded-lg border border-border bg-popover text-popover-foreground shadow-xl animate-in fade-in-0 zoom-in-95 p-1" style={{ top: contextMenu.y, left: contextMenu.x }}>
                    <button onClick={() => editNode(contextMenu.nodeId)} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm hover:bg-accent transition-colors"><Pencil className="h-3.5 w-3.5" /> Edit</button>
                    <button onClick={() => deleteNode(contextMenu.nodeId)} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-destructive hover:bg-destructive/10 transition-colors"><Trash2 className="h-3.5 w-3.5" /> Delete</button>
                </div>
            )}

            <ScheduleDialog open={showSchedule} onClose={() => setShowSchedule(false)} cronExpression={cronExpression} scheduleEnabled={scheduleEnabled} onSave={async (cron, enabled) => {
                setCronExpression(cron); setScheduleEnabled(enabled);
                try {
                    const res = await updateScheduleAction({ workflowId, cron, enabled });
                    if (res.error) toast.error(res.error); else toast.success("Schedule updated");
