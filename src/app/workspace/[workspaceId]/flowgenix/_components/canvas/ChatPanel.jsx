...
export default function ChatPanel({
  ...
  return (
    <div className="flex h-full bg-[#121212] border-t border-white/5 shadow-2xl relative overflow-hidden font-sans text-white">
      {/* Left: Chat */}
      <div className="flex flex-col w-[380px] border-r border-white/5 bg-black/20">
        <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-[#1a1a1a]">
          <div className="flex items-center gap-2">
            <MessageSquare className="h-3.5 w-3.5 text-orange-500" />
            <span className="text-xs font-bold uppercase tracking-wider text-white/80">Chat</span>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 min-h-0 scrollbar-thin scrollbar-thumb-white/10">
          {messages.length === 0 && (
            <div className="flex flex-col items-center justify-center h-full text-center space-y-2 opacity-20">
              <MessageSquare className="h-10 w-10" />
              <p className="text-[10px] uppercase tracking-widest">Awaiting Message...</p>
            </div>
          )}
          {messages.map((msg) => (
        ...
        <div className="p-3 border-t border-white/5 bg-black/40 space-y-2">
          <div className="flex items-center justify-between px-1">
             <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-bold text-white/30 uppercase tracking-tighter">AI Tools</span>
             </div>
             <Switch
      ...
      <div className="flex flex-col w-[280px] border-r border-white/5 bg-black/10">
        <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-[#1a1a1a]">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-bold uppercase tracking-wider text-white/80">Logs</span>
          </div>
            ...
            >
              <div className="flex items-center justify-between mb-1">
                <div className="flex items-center gap-2">
                   ...
                   <span className="text-[10px] font-bold text-white/70 uppercase tracking-tight">{log.nodeName}</span>
                </div>
        ...
        <div className="flex items-center justify-between px-4 py-2 border-b border-white/5 bg-[#1a1a1a]">
          <div className="flex items-center gap-3">
             <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs font-bold uppercase tracking-wider text-white/70">{selectedLog?.nodeName || 'Inspector'}</span>
             </div>
             {selectedLog && (
          ...
          {selectedLog ? (
            <div className="space-y-4">
               <div className="flex items-center gap-2 text-white/20">
                  ...
                  <span className="text-[9px] uppercase tracking-tighter font-bold">{viewMode} DATA</span>
                  <div className="h-px flex-1 bg-white/5" />
               </div>
            ...
            <div className="flex flex-col items-center justify-center h-full opacity-10">
               ...
               <p className="text-xs uppercase tracking-[0.2em]">Select Trace Node</p>
            </div>
          )}
        </div>
      </div>
