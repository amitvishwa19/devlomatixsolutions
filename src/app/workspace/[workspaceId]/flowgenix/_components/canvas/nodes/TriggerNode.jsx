...
export const TriggerNode = memo(({ id, data }) => {
  const Icon = data?.icon ?? Zap;
  return (
    <div className="group relative">
      <NodeDeleteButton onDelete={() => data?.onDelete?.(id)} />
      <div className={`flex w-[220px] items-center gap-3 rounded-md border-l-4 border-l-primary border border-border bg-card px-3 py-2.5 shadow-sm transition-all ${statusRingClass(data?.runStatus)}`}>
