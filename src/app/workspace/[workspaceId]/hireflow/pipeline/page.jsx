...
export default function CandidatePipelinePage() {
    ...
    return (
        <div className="flex flex-col h-[calc(100vh-4rem)] overflow-hidden">
            ...
            {/* Kanban Board — the outer div scrolls horizontally only, columns scroll vertically */}
            {isMounted ? (
                    ...
                    <div className="flex-1 overflow-x-auto overflow-y-hidden px-8 pb-8 custom-scrollbar">
                        <div className="flex gap-6 h-full">
                            {STAGE_MAP.map((stage) => {
                                ...
                                return (
                                    <div key={stage.id} className="w-[320px] shrink-0 flex flex-col gap-4 h-full">
                                        {/* Stage Header */}
                                        ...
                                        {/* Droppable Column — THIS is the only scroll container for each column */}
                                        <Droppable droppableId={stage.id}>
                                            {(provided, snapshot) => (
                                                <div
                                                    ref={provided.innerRef}
                                                    {...provided.droppableProps}
                                                    className={`flex-1 rounded-md p-4 space-y-4 overflow-y-auto transition-all duration-300 min-h-[120px] ${
                                                        snapshot.isDraggingOver
                                                            ? `bg-muted/40 border-2 border-dashed ${stage.accent}`
                                                        ...
                                                        >
                                                            {(dragProvided, dragSnapshot) => (
                                                                <div
                                                                    ...
                                                                    className={`p-5 rounded-md bg-card/60 backdrop-blur-xl border border-border/40 shadow-xl shadow-black/5 cursor-grab group hover:border-primary/40 transition-all select-none ${
                                                                        ...
                                                                        <div className="flex items-center gap-3">
                                                                            ...
                                                                            <div>
                                                                                <h4
                                                                                    ...
                                                                                    onClick={(e) => {
                                                                                        e.stopPropagation();
                                                                                        router.push(`/workspace/${workspaceId}/ats/candidates/${candidate.candidateId}`);
                                                                                    }}
                                                                                >
                                                                                    {candidate.name}
                                                                                </h4>
