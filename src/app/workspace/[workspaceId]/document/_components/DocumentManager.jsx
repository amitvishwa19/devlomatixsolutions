...
const FileTypeIcon = ({ document }) => {
    ...
    if (isFolder) return (
        <div className="w-12 h-12 rounded-xl bg-amber-50 flex items-center justify-center border border-amber-100 shadow-sm">
    ...
    return (
        <div className={`w-12 h-12 rounded-xl ${bgColor} flex items-center justify-center border border-border/50 shadow-sm`}>
    ...
    return (
        <div className="space-y-6">
            <div className="space-y-2">
                <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Recipient Email</Label>
    ...
    return (
        <div className="flex flex-col h-full gap-2">
            ...
            <Dialog open={isFolderOpen} onOpenChange={setIsFolderOpen}>
                <DialogContent className="sm:max-w-[425px] rounded-3xl overflow-hidden border-border/40 shadow-2xl p-0">
                    ...
                    <div className="px-8 py-4">
                        <div className="space-y-2">
                            <Label className="text-[10px] font-black uppercase tracking-widest text-muted-foreground ml-1">Folder Name</Label>
                            <Input
                                placeholder="Enter folder name..."
                                value={newFolderName}
                                onChange={(e) => setNewFolderName(e.target.value)}
                                className="h-12 bg-muted/30 border-none shadow-none focus-visible:ring-2 focus-visible:ring-primary/20 rounded-2xl font-bold"
                            />
                        </div>
                    </div>
            ...
            <Dialog open={isUploadOpen} onOpenChange={setIsUploadOpen}>
                <DialogContent className="sm:max-w-[500px] rounded-3xl overflow-hidden border-border/40 shadow-2xl p-0">
                    <DialogHeader className="p-8 pb-4">
                        ...
                        <div className="relative group cursor-pointer border-2 border-dashed border-border/40 rounded-[2rem] bg-muted/5 p-12 text-center transition-all hover:bg-primary/[0.02] hover:border-primary/30">
                            ...
                            <div className="flex flex-col items-center gap-4 group-hover:scale-105 transition-all duration-300">
                                ...
                                <div className="space-y-1">
                                    ...
                                    <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-[0.2em] opacity-60">PDF • IMAGE • VIDEO • AUDIO</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <DialogHeader className="p-8 border-b border-border/20 flex flex-row items-center justify-between shrink-0 bg-background/50">
                        <div className="flex items-center gap-5">
                            ...
                            <div className="flex flex-col">
                                <DialogTitle className="text-2xl font-black truncate max-w-[40vw] tracking-tight">
                                    {previewDoc?.name}
                                </DialogTitle>
                                <div className="flex items-center gap-3 text-[10px] font-black text-muted-foreground uppercase tracking-[0.15em] mt-1 italic">
                            ...
                            if (mType.startsWith("audio/")) {
                                return (
                                    <div className="bg-background/80 backdrop-blur-3xl p-16 rounded-[3rem] border border-border/20 shadow-[0_48px_96px_-24px_rgba(0,0,0,0.12)] flex flex-col items-center gap-12 w-full max-w-lg relative overflow-hidden group">
                                        ...
                                        <p className="text-[10px] text-muted-foreground font-black uppercase tracking-[0.3em] italic relative z-10 opacity-40">{previewDoc.name}</p>
                                    </div>
                                );
                            }
                            ...
                            return (
                                <div className="flex flex-col items-center justify-center py-20 text-center max-w-md">
                                    <div className="w-32 h-32 bg-muted/30 rounded-[2.5rem] flex items-center justify-center mb-10 relative group ring-1 ring-border/50 shadow-inner">
                                        ...
                                        <div className="absolute -bottom-2 -right-2 w-12 h-12 bg-background border border-border/40 rounded-2xl flex items-center justify-center text-[11px] font-black text-primary uppercase shadow-2xl ring-4 ring-muted/20">
