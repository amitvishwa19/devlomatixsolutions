...
export default function FolderDetailsModal({ isOpen, onOpenChange, folder, workspaceId, onDelete }) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={onOpenChange}>
            <DialogContent className="sm:max-w-2xl rounded-2xl border-none shadow-2xl overflow-hidden p-0 bg-card">
                <div className="flex flex-col md:flex-row h-[500px]">
                    ...
                    <div className="w-full md:w-[280px] bg-muted/10 border-r border-border/40 p-6 flex flex-col justify-between shrink-0">
                        <div>
                            <div className="flex items-center gap-4 mb-6">
                                <div className={`h-14 w-14 rounded-2xl flex items-center justify-center shadow-sm border border-border/10 ${folder.color || 'bg-blue-50 text-blue-500'}`}>
                                    <FolderOpen className="h-7 w-7" />
                                </div>
                                <div className="flex-1 min-w-0">
                                    <DialogTitle className="text-xl font-black text-foreground tracking-tight truncate">{folder.name}</DialogTitle>
                                    <DialogDescription className="sr-only">Details and contents for {folder.name}</DialogDescription>
                                    <Badge variant="outline" className={`${statusStyles[folder.status?.toLowerCase()] || statusStyles.approved} border-none shadow-sm text-[10px] uppercase font-black tracking-widest mt-1`}>
                            ...
                            <div className="space-y-5">
                                <div className="space-y-1">
                                    <p className="text-xs font-black uppercase tracking-widest text-muted-foreground/60">Owner</p>
                                ...
                                <div className="space-y-1">
                                    <p className="flex items-center gap-2 text-xs font-black uppercase tracking-widest text-muted-foreground/60">
                                ...
                                <div className="flex gap-4">
                                    <div className="space-y-1">
                                        <p className="flex items-center gap-1.5 text-xs font-black uppercase tracking-widest text-muted-foreground/60">
                    ...
                    <div className="flex-1 flex flex-col bg-background relative">
                        <div className="p-4 border-b border-border/40 bg-muted/5">
                            <h3 className="text-sm font-black uppercase tracking-widest text-muted-foreground px-2">Contents Preview</h3>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto p-2">
                            {isLoading ? (
                                <div className="h-full flex flex-col items-center justify-center text-muted-foreground opacity-50 gap-3">
                                    ...
                                    <p className="text-xs font-bold tracking-widest uppercase">Loading items...</p>
                                </div>
                            ) : files.length === 0 ? (
                                <div className="h-full flex flex-col items-center justify-center text-muted-foreground opacity-50 gap-4">
                                    ...
                                    <p className="text-xs font-bold tracking-widest uppercase">This folder is empty</p>
                                </div>
                            ) : (
                                <div className="space-y-1">
                                    {files.map(file => (
                                        <div key={file.id} className="flex items-center justify-between p-3 rounded-xl hover:bg-muted/40 transition-colors group cursor-default">
                                            <div className="flex items-center gap-3 overflow-hidden">
                                                ...
                                                <div className="min-w-0 flex-1">
                                                    ...
                                                    <p className="text-[11px] font-semibold text-muted-foreground uppercase opacity-80 mt-0.5">
