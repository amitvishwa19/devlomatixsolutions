                ...
                <form onSubmit={onSubmit} className="flex flex-col">
                    ...
                    <div className="p-8 space-y-6 max-h-[60vh] overflow-y-auto">
                        <div className="space-y-4">
                            ...
                            <div className="space-y-2 text-left">
                                <label className="text-xs font-bold text-muted-foreground ml-1 flex items-center gap-1.5 uppercase tracking-widest opacity-60">
                                    Category Slug
                                </label>
                                <Input
                                    disabled={isLoading}
                            ...
                            <div className="space-y-2 text-left">
                                <label className="text-xs font-bold text-muted-foreground ml-1 flex items-center gap-1.5 uppercase tracking-widest">
                                    <FolderTree size={12} /> Parent Category
                                </label>
                                <Select value={parentId} onValueChange={setParentId} disabled={isLoading}>
                                    <SelectTrigger className="h-12 bg-muted/30 border-none rounded-md focus-visible:ring-1 focus-visible:ring-primary shadow-inner text-xs font-bold">
                                        <SelectValue placeholder="None (Top-level)" />
                                    </SelectTrigger>
                                    <SelectContent className="rounded-md border-border/40 bg-card/90 backdrop-blur-xl">
                                        <SelectItem value="none" className="font-black text-xs rounded-md uppercase tracking-widest">
                                            NONE (TOP-LEVEL CATEGORY)
                                        </SelectItem>
                                        {parentCategories?.map((parent) => (
                                            <Fragment key={parent.id}>
                                                <SelectItem value={parent.id} className="font-bold text-xs rounded-md">
                                                ...
                                                {parent.children?.map(child => (
                                                    <SelectItem key={child.id} value={child.id} disabled={child.id === category?.id} className="font-bold text-xs rounded-md pl-8">
                    ...
                    <DialogFooter className="p-8 pt-0 flex gap-3">
                        <Button
                            ...
                            variant="outline"
                            onClick={handleClose}
                            className="h-12 rounded-md font-bold flex-1 border-border bg-background/50 text-xs"
                        >
                            Cancel
                        </Button>
                        <Button
                            disabled={isLoading}
                            className="h-12 rounded-md font-bold flex-1 bg-primary hover:bg-primary/90 shadow-lg shadow-primary/20 text-xs"
                        >
                            {isLoading ? (
