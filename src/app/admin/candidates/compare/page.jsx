...
export default function CandidateComparePage() {
  ...
  const allSkills = Array.from(new Set(selected.filter(Boolean).flatMap((c) => c.skills)));
            ...
            {selected.map((c, i) =>
              c ? (
                ...
                <Card key={i} className="border-dashed">
                  <CardContent className="p-4 flex items-center justify-center min-h-[200px] h-full text-sm text-muted-foreground">
                    Select a candidate
                  </CardContent>
                </Card>
              )
