...
import { Search, User, Briefcase, Calendar, ArrowRight } from "lucide-react";
...
const GlobalSearch = () => {
  ...
  const results = query.trim().length < 2 ? [] : [
    ...candidates
      ...
      .slice(0, 5)
      .map((c) => ({ id: c.id, type: "candidate", title: c.name, subtitle: `${c.jobTitle} · ${c.stage}`, link: `/admin/candidates/${c.id}` })),
    ...jobs
      .filter((j) => j.title.toLowerCase().includes(query.toLowerCase()) || j.department.toLowerCase().includes(query.toLowerCase()))
      .slice(0, 3)
      .map((j) => ({ id: j.id, type: "job", title: j.title, subtitle: `${j.department} · ${j.location} · ${j.applicants} applicants`, link: `/admin/jobs` })),
    ...interviews
      .filter((i) => i.candidateName.toLowerCase().includes(query.toLowerCase()) || i.jobTitle.toLowerCase().includes(query.toLowerCase()))
      .slice(0, 3)
      .map((i) => ({ id: i.id, type: "interview", title: `${i.candidateName} - ${i.type}`, subtitle: `${i.jobTitle} · ${i.date} ${i.time}`, link: `/admin/interviews` })),
  ];
