...
interface CredentialSelectorProps {
  ...
  /** Filter to specific credential types, e.g. ["openai", "slack"] */
  types?: string[];
  label?: string;
}

// Maps node types to credential types they accept
export const NODE_CREDENTIAL_TYPES: Record<string, string[]> = {
...
export default function CredentialSelector({ value, onChange, types, label = "Credential" }: CredentialSelectorProps) {
  ...
  useEffect(() => {
    const fetch = async () => {
      setLoading(true);
      let query = supabase.from("credentials").select("id, name, credential_type");
      if (types && types.length > 0) {
        query = query.in("credential_type", types);
      }
