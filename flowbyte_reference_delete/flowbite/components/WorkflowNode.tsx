...
type WorkflowNodeData = {
  ...
  attachedSlots?: Record<string, string>;
...
const WorkflowNode = ({ data, selected }: NodeProps & { data: WorkflowNodeData }) => {
  ...
  const agentConfig = (nodeData as any).config as Record<string, unknown> | undefined;
  ...
  const status: ExecutionStatus = nodeData.status
    ? nodeData.status
    : nodeData.error
      ? "error"
      : nodeData.executed
    ...
    return (
      <div className="flex flex-col items-center gap-1.5 cursor-pointer group">
        <div className="relative">
          <div
            className={`w-[72px] h-[72px] rounded-2xl border bg-[hsl(var(--n8n-node-bg))] flex items-center justify-center transition-all
      ...
      <div className="flex flex-col items-center cursor-pointer group">
        ...
        <div className="flex items-start gap-6 mt-0">
          {AGENT_SLOTS.map((slot) => {
            ...
            const builtinLabel =
              slot.id === "llm" && hasBuiltinLlm
                ? (agentConfig?.builtinLlm as string) || "gpt-4"
                : slot.id === "memory" && hasBuiltinMemory
                  ? (agentConfig?.builtinMemory as string)
                  : undefined;
  ...
  return (
    <div
      className={`workflow-node rounded-xl border bg-card transition-all cursor-pointer min-w-[160px] ${executionClass}
