...
export const Route = createFileRoute("/careers")({
  ...
  head: () => ({
    meta: [
      { title: "Careers — KonnectX WhatsApp Business API" },
      { name: "description", content: "Join KonnectX and help shape the future of business messaging. Explore open positions across engineering, design, and more." },
      { property: "og:title", content: "Careers — KonnectX" },
      { property: "og:description", content: "Join KonnectX and help shape the future of business messaging." },
    ],
  }),
});
            ...
            <div className="mt-8 space-y-4">
              {openings.map((job, i) => (
                <motion.div key={job.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.08, duration: 0.5 }}>
                  <Link to="/contact" className="group flex items-center justify-between glass-card rounded-xl p-6 transition-all hover:ring-2 hover:ring-primary/30" style={{ boxShadow: "var(--shadow-card)" }}>
                    <div>
