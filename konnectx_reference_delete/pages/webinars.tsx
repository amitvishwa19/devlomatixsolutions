...
export const Route = createFileRoute("/webinars")({
  ...
  head: () => ({
    meta: [
      { title: "Webinars — KonnectX WhatsApp Business API" },
      { name: "description", content: "Join live webinars and watch recordings on WhatsApp marketing, automation, and business growth strategies." },
      { property: "og:title", content: "Webinars — KonnectX" },
      { property: "og:description", content: "Live and on-demand webinars on WhatsApp marketing and automation." },
    ],
  }),
});
            ...
            <div className="mt-8 space-y-4">
              {upcoming.map((w, i) => (
                <motion.div key={w.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: i * 0.1, duration: 0.5 }} className="glass-card rounded-xl p-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4" style={{ boxShadow: "var(--shadow-card)" }}>
                  ...
                  <Button size="sm" style={{ background: "var(--gradient-sun)" }} asChild><Link to="/contact">Register</Link></Button>
                </motion.div>
              ))}
            </div>
