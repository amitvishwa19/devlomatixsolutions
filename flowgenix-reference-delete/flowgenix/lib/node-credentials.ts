...
export function splitConfig(
  ...
  config: Record<string, unknown>,
): { plain: Record<string, unknown>; secrets: Record<string, unknown> } {
...
export function hasSecretFields(kind: string | undefined): boolean {
  return Boolean(kind && (SECRET_FIELDS[kind]?.length ?? 0) > 0);
}

// ---------- CRUD ----------

export async function listCredentials(kind?: string): Promise<NodeCredentialRow[]> {
  let q = supabase.from("node_credentials").select("*").order("updated_at", { ascending: false });
  if (kind) q = q.eq("kind", kind);
  const { data, error } = await q;
  if (error) throw error;
  return (data ?? []) as NodeCredentialRow[];
}

export async function getSecret(secretId: string): Promise<NodeSecretRow | null> {
  const { data, error } = await supabase
    ...
    .maybeSingle();
  if (error) throw error;
  return (data as NodeSecretRow | null) ?? null;
}
...
export async function saveCredentialPreset(opts: {
  ...
  if (Object.keys(secrets).length > 0) {
    const { data: s, error: se } = await supabase
      .from("node_secrets")
      .insert({ kind: opts.kind, name: opts.name, secrets: secrets as never })
      .select("*")
      .single();
    if (se) throw se;
    secretId = (s as NodeSecretRow).id;
  }
  const { data, error } = await supabase
    .from("node_credentials")
    .insert({ kind: opts.kind, name: opts.name, config: plain as never, secret_id: secretId })
    .select("*")
    .single();
  if (error) throw error;
  return data as NodeCredentialRow;
...
export async function deleteCredential(id: string): Promise<void> {
  ...
  const secretId = (cred as { secret_id: string | null } | null)?.secret_id;
...
export async function applyCredential(cred: NodeCredentialRow): Promise<Record<string, unknown>> {
  const merged: Record<string, unknown> = { ...cred.config };
  if (cred.secret_id) {
