...
import { createWorkflow, listWorkflows, saveWorkflow, type WorkflowRow } from "@/flowgenix/lib/workflow-storage";
...
export const TemplatesDialog = () => {
  ...
  const [previewing, setPreviewing] = useState<WorkflowRow | null>(null);
  ...
  const useTemplate = async (tpl: WorkflowRow) => {
    try {
                ...
                {previewing?.id === tpl.id && (
                  <div className="mt-3 max-h-40 overflow-auto rounded border border-border bg-secondary/30 p-2">
                    <ul className="space-y-1">
                      {(tpl.nodes ?? []).map((n) => {
                        const d = n.data as { label?: string; kind?: string };
                        return (
