...
import { testNode, type TestResult } from "@/flowgenix/lib/node-test";
...
export const NodeCredentialsSection = ({ kind, config, onApplyConfig }: Props) => {
  const [presets, setPresets] = useState<NodeCredentialRow[]>([]);
  ...
  const usePreset = async (id: string) => {
    setSelectedId(id);
    if (!id) return;
  ...
  const runTest = async () => {
    ...
    setTestResult(null);
    try {
      let parsed: unknown = sampleInput;
      try {
