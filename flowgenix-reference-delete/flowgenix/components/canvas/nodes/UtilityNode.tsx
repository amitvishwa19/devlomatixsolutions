...
export const UtilityNode = memo(({ id, data }: NodeProps) => {
  const d = data as Data;
  const Icon = d.icon ?? Wrench;
  const isIf = d.kind === "util.if";
  const hasRetry = (d.retry?.count ?? 0) > 0;
  ...
  return (
    <div className="group relative">
      ...
      <div className={`flex w-[220px] items-center gap-3 rounded-md border border-border bg-card px-3 py-2.5 shadow-sm transition-all ${statusRingClass(d.runStatus)}`}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="font-mono text-[10px] uppercase tracking-wider text-muted-foreground">
            {isIf ? "branch" : "action"}{hasRetry && ` · ↻${d.retry?.count}`}
          </p>
        </div>
      </div>
