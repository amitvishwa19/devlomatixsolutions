...
import { stripSecretsForStorage } from "@/flowgenix/lib/node-credentials";
...
interface Props {
  ...
  onSave: (id: string, patch: { label?: string; description?: string; config?: Record<string, unknown>; retry?: RetryConfig }) => void;
  onDelete: (id: string) => void;
  webhookToken?: string | null;
  onRegenerateWebhookToken?: () => Promise<string | null>;
}
...
export const NodeSettingsSheet = ({
  ...
  const [retry, setRetry] = useState<RetryConfig>({ count: 0, delayMs: 500 });
  ...
  const [lastIO, setLastIO] = useState<{ input: unknown; output: unknown; status: string; created_at: string; message: string | null } | null>(null);

  const fetchLastIO = async (nodeId: string) => {
    ...
    try {
      ...
      const d = (row.data ?? {}) as { input?: unknown; output?: unknown };
  ...
  useEffect(() => {
    ...
    setTestResult(null);
    setLastIO(null);
    const d = node.data as { label?: string; description?: string; config?: Record<string, unknown>; retry?: RetryConfig };
    setLabel(d.label ?? "");
        ...
        <Tabs defaultValue="settings" className="mt-4">
          ...
          {kind === "util.email" && (
            <>
              ...
              <p className="font-mono text-[10px] text-muted-foreground">// Sent via Resend connector. Link Resend in Lovable Cloud → Connectors to enable.</p>
            </>
          )}
...
const formatIO = (v: unknown): string => {
...
const IOPanel = ({
  ...
  which: "input" | "output";
  loading: boolean;
  lastIO: { input: unknown; output: unknown; status: string; created_at: string; message: string | null } | null;
  onRefresh: () => void;
}) => {
