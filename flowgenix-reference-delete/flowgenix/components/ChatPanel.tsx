...
import { runAgent, AgentAbortError } from "@/flowgenix/lib/agent-runtime";
import {
  ChatThread,
  appendThreadMessage,
  clearThreadMessages,
...
import { ChatThreadList } from "@/flowgenix/components/ChatThreadList";
...
function parseTraces(meta?: string): ToolCallTrace[] {
  if (!meta) return [];
  try {
...
const ToolCallBlock = ({ trace }: { trace: ToolCallTrace }) => {
...
export const ChatPanel = ({ config, ragDocs }: Props) => {
  ...
  const [liveTraces, setLiveTraces] = useState<ToolCallTrace[]>([]);
  ...
  useEffect(() => {
    const vp = scrollRef.current?.querySelector<HTMLDivElement>(
  ...
  /** Run the agent with a given user message; appends both messages on success. */
  const runWith = async (text: string, baseHistory: ChatMessage[]) => {
    if (!activeId) return;
    ...
    try {
      ...
      const assistantMsg: ChatMessage = { role: "assistant", content: reply, meta };
  ...
  const regenerate = async () => {
    ...
    try {
      ...
      // Simpler: just keep the user msg in DB, and pass baseHistory + lastUserText through runWith,
      // but that would duplicate the user msg. So delete it too.
        ...
        <div className="border-t border-border bg-card/40 p-3">
          ...
          <div className="flex gap-2">
            <Textarea
              ...
              disabled={busy}
            />
            <Button onClick={send} disabled={busy || !input.trim()} className="font-mono self-end">
